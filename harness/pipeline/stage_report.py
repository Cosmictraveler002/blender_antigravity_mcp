...
def run_report(project: ProjectDefinition, config: HarnessConfig) -> Dict[str, Any]:
    ...
    if refinement:
        ...
        total_iters = refinement.get("total_passes")
        if total_iters is None:
            total_iters = refinement.get("geometry_iterations", 0) + refinement.get("color_iterations", 0)
        report["summary"]["total_iterations"] = total_iters
        report["summary"]["refinement_score"] = refinement.get("final_score", 0.0)
        report["summary"]["geometry_score"] = refinement.get("final_geometry_score", refinement.get("final_score", 0.0))
        report["summary"]["color_score"] = refinement.get("final_color_score", 0.0)
        report["summary"]["refinement_status"] = refinement.get("status", "converged" if refinement.get("converged") else "completed")
...
def _write_markdown_report(path: str, report: Dict, project: ProjectDefinition,
    ...
    if refinement:
        ...
        lines.append("")
        total_passes = refinement.get("total_passes", refinement.get("geometry_iterations", 0) + refinement.get("color_iterations", 0))
        final_score = refinement.get("final_score", 0.0)
        is_converged = refinement.get("converged", False)
        status_str = refinement.get("status", "Yes" if is_converged else "No").capitalize()

        lines.append("| Phase | Passes / Iterations | Final Score | Status / Converged |")
        lines.append("|:---|:---|:---|:---|")
        lines.append(f"| Unified Closed-Loop | {total_passes} | {final_score:.2f}% | {status_str} |")

        geom_s = refinement.get("final_geometry_score")
        col_s = refinement.get("final_color_score")
        tex_s = refinement.get("final_texture_score")
        if geom_s is not None or col_s is not None:
            lines.append("")
            lines.append(f"**Score Breakdown**: Geometry `{geom_s or 0.0:.1f}%` | Color `{col_s or 0.0:.1f}%` | Texture `{tex_s or 0.0:.1f}%`")
        lines.append("")
    ...
    if comparison and "recommendations" in comparison:
        ...
        for i, rec in enumerate(comparison["recommendations"], 1):
            if isinstance(rec, dict):
                act = rec.get("action", str(rec))
                prio = f"**[{rec.get('priority', 'INFO')}]** " if "priority" in rec else ""
                lines.append(f"{i}. {prio}{act}")
            else:
